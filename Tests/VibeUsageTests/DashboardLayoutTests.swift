...
struct DashboardLayoutTests {
    ...
    func walletManagementUsesNativeSubscriptionRechargeAndFundingSections() throws {
        ...
        #expect(view.contains("private var subscriptionEmptyState"))
        #expect(view.contains("maxHeight: .infinity, alignment: .center"))
        #expect(view.contains("multilineTextAlignment(.center)"))
        ...
        #expect(view.contains("walletRechargeAmountFieldWidth"))
        #expect(view.contains("walletRechargeButtonWidth"))
        #expect(!view.contains("预计支付金额"))
        #expect(!view.contains("请先计算"))
        #expect(!view.contains("计算实付"))
        #expect(!view.contains("支付二维码将在软件内显示"))
